...
飏莜勋蜕噼册鉅宫疱侣撸硚锩鸴锳游魇冢垧桠戬于袯屃拼".Replace("\n", "").Trim();

        static string stts_t = @"
...
颺蓧勳蛻劈冊钜宮皰侶擼礄錈鴬鍈遊魘塚坰椏戩於襏屭拚".Replace("\n", "").Trim();
        ...
        public static string stts_big52gb(string input)
        {
            string[] s = stringToStringArray(input);
            for (int i = 0, max_i = s.Count(); i < max_i; i++)
            {
                var index = stts_t.IndexOf(s[i]);
                if (index != -1)
                {
                    s[i] = stts_s.Substring(index, 1);
                }
            }
            return string.Join("", s);
        }
        public static string stts_gb2big5(string input)
        {
            string[] s = stringToStringArray(input);
            for (int i = 0, max_i = s.Count(); i < max_i; i++)
            {
                var index = stts_s.IndexOf(s[i]);
                if (index != -1) s[i] = stts_t.Substring(index, 1);
            }
            return string.Join("", s);
        }
    }
